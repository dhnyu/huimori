...
tuning_results_list <- furrr::future_map(all_targets, function(target) {
  ...
  return(metrics)
     ...
     packages = c("tidymodels", "spatialsample", "finetune", "xgboost", "dplyr", "tidyr", "sf")
   ))
...
write.csv(summary_pm10, 
          file = file.path(target_dir, "summary_performance_era5_pm10.csv"), 
          row.names = FALSE)

write.csv(summary_pm25, 
          file = file.path(target_dir, "summary_performance_era5_pm25.csv"), 
          row.names = FALSE)
```
...
write.csv(summary_pm10, 
          file = file.path(target_dir, "summary_performance_chelsa_pm10.csv"), 
          row.names = FALSE)

write.csv(summary_pm25, 
          file = file.path(target_dir, "summary_performance_chelsa_pm25.csv"), 
          row.names = FALSE)
```
