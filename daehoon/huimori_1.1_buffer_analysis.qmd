...
```{r}
# 1. 병렬 처리 설정 (mirai_multisession 적용)
# 200만 행의 데이터 크기를 고려하여
future::plan(future.mirai::mirai_multisession, workers = 20L)

# 2. 데이터 준비 및 피처 엔지니어링 (data.table 활용)
...
# 3. 공간적 블록 교차 검증 (Spatial Block CV)
set.seed(123)
spatial_folds <- spatial_block_cv(df_train_full, v = 5)

# 4. 모델 레시피 및 사양 정의
xgb_recipe <- recipe(PM10_smooth ~ ., data = df_train_full) %>%
  update_role(TMSID, TMSID2, year, month, day, geometry, starts_with("date"), new_role = "ID") %>%
...
xgb_spec <- boost_tree(
  ...
  set_engine("xgboost", nthread = 1) %>% # 전체 병렬 처리를 future가 관리하므로 내부 nthread는 1로 설정
  set_mode("regression")

xgb_workflow <- workflow() %>% add_model(xgb_spec) %>% add_recipe(xgb_recipe)

# 5. 하이퍼파라미터 튜닝 (Race ANOVA)
model_metrics <- metric_set(rmse, mae)

# Race ANOVA를 통한 고속 튜닝 수행
tune_res <- tune_race_anova(
  xgb_workflow,
  resamples = spatial_folds,
  grid = 20, # Latin Hypercube 기반 자동 그리드 생성
  metrics = model_metrics,
  control = control_race(
    parallel_over = "everything", # future 프레임워크 활용 설정
    save_pred = TRUE,
    verbose_elim = TRUE
  )
)
...
# 6. 최종 모델 확정 및 2023년 외부 검증
best_params <- select_best(tune_res, metric = "rmse")
final_fit <- finalize_workflow(xgb_workflow, best_params) %>% fit(data = df_train_full)

results_2023 <- predict(final_fit, df_test_2023) %>%
  bind_cols(df_test_2023) %>%
  st_drop_geometry()

final_performance <- results_2023 %>% model_metrics(truth = PM10_smooth, estimate = .pred)
